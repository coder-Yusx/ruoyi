...
public class GgfcHbController extends BaseController {
	...
	@PostMapping("/list")
	@ResponseBody
	public TableDataInfo list(GgfcHb ggfcHb) throws UnsupportedEncodingException {
	...
	public AjaxResult editDataCleanSave(GgfcHb ggfcHb) throws Exception {
		...
		tDataCheck.setTableId(ggfcHb.getId());
	...
	@Log(title = "分配公共户", businessType = BusinessType.DISTRIBUTE)
	...
	@Log(title = "审核公共户", businessType = BusinessType.EXAMINE)
	@PostMapping("/examine")
	@ResponseBody
	public AjaxResult examine(GgfcHb ggfcHb) {
		...
		//审核通过时执行数据库中SQL
		if (ggfcHb.getCheckStat().equals("2")) {
		 List<TDataCheck> list= tDataCheckService.selectTDataCheckByTableId(ggfcHb.getId());
		for (int i = 0; i <list.size() ; i++) {
			if (Common.notEmpty(list.get(i).getExt3())) {
				tDataCheckService.updatesql(list.get(i).getExt3());
			}
		} }
		//审核所有记录id相关记录
	 ...
	 */
	@RequiresPermissions("basp:ggfcHb:examine")
	@Log(title = "审核权利人", businessType = BusinessType.EXAMINE)
	@PostMapping("/examineQlr")
	@ResponseBody
	public AjaxResult examineQlr(GgfcHb ggfcHb) {
		TDataCheck tDataCheck = new TDataCheck();
		//获取当前登录用户
		SysPerson currentUser = ShiroUtils.getSysUser();
		...
		return toAjax(tDataCheckService.updateById(tDataCheck));
	}
	...
	public String examineid(@PathVariable("id") String id, ModelMap mmap) {
		List<TDataCheck> tDataCheck= tDataCheckService.selectTDataCheckByTableId(id);//修改记录表
		GgfcHb ggfcHb = ggfcHbService.selectGgfcHbById(id);                    //公共户表
		List<CyztCyjgKfs> cyztCyjgKfs = cyztCyjgKfsService.selectCyztCyjgKfsByHbId(id);            //从业主体_开发商表
		List<XmglXmXmjbxxb> xmglXmXmjbxxb = xmglXmXmjbxxbService.selectXmglXmXmjbxxbByHbId(id);        //项目基本信息表
		List<GgfcLb> ggfcLb = ggfcLbService.selectGgfcLbByHbId(id);            //公共楼表
