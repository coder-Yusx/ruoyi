...
(function ($) {
    $.extend({
    	...
    	modal: {
            ...
            //弹窗
            openexamine: function (title, url, width, height, callback) {
                //如果是移动端，就使用自适应大小弹窗
                ...
                var arry = [];
                var idindexvalue = url.substring(url.lastIndexOf("/")+1,url.length);
                $("#bootstrap-table>tbody>tr").each(function(){
                    arry.push($(this).find("td:eq(1)").text());
                })
                var shunxuindex1 = layer.open({
                    type: 2,
                    ...
                    title: title,
                    content: url,
                    btn: ['上一条', '下一条', '审核通过','审核不通过', '关闭'],
                    shadeClose: true,	// 弹层外区域关闭
                    yes: function(){
                        if(arry.indexOf(idindexvalue) == 0){
                            layer.msg("已经到顶了", {icon: 7});
                            return false;
                        }else{
                            $.operate.examine(arry[arry.indexOf(idindexvalue)-1]);
                        }
                    },
                    btn2: function(){
                        if(arry.indexOf(idindexvalue) == arry.length-1){
                            layer.msg("已经到底了", {icon: 7});
                            return false;
                        }else{
                            $.operate.examine(arry[arry.indexOf(idindexvalue)+1]);
                        }
                    ...
                    btn3:  function(index,layero){
                        ...
                        //获取当前id 审核通过
                        var data = {"id":arry[arry.indexOf(idindexvalue)] ,"ext10":'2' ,"checkStat":'2'};
						//公共户提交
                        var url = $.table._option.examineUrl;
                        $.operate.saveTabE(url, data);
                        //权利人提交审核
                        var urlQlr = $.table._option.examineQlr;
                        iframeWin.$('.form-djGgQlrmx-edit').each(function(){
                            var checkId =$(this).find("#qlid").val();
                            var dataQlr = {"checkId":checkId,"checkStat":'2'};
                            $.operate.saveTabE(urlQlr, dataQlr);
                        })
                    },
                    btn4: function(index,layero){
                        var iframeWin = window[layero.find('iframe')[0]['name']];//得到iframe页的窗口对象，执行iframe页的方法：
                        var data = {"id":arry[arry.indexOf(idindexvalue)] ,"ext10":'3' ,"checkStat":'3' };
                        //公共户提交 审核不通过
                        var url = $.table._option.examineUrl;
                        $.operate.saveTabE(url, data);
						//权利人提交审核$(this).
                        ...
                        iframeWin.$('.form-djGgQlrmx-edit').each(function(){
                            var checkId =$(this).find("#qlid").val();
                            var dataQlr = {"checkId":checkId,"checkStat":'3'};
                            $.operate.saveTabE(urlQlr, dataQlr);
                        })
                    },
                    btn5: function(){
                ...
                layer.full(shunxuindex1);
            },
            ////弹窗opendistribute
            opendistribute: function (title, url, width, height, callback) {
                //如果是移动端，就使用自适应大小弹窗
                if (navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)) {
                    width = 'auto';
                    height = 'auto';
                ...
                layer.open({
                    ...
                    yes: function(index,layero){
                        ...
                        //分配确认
                        // 获取url中"#"符后的字串
                        var nurl = iframeWin.window.location.href;
                        if(nurl.indexOf("#") != -1){
                            var theRequest = new Object();
                            var stt="";
                            var str = nurl.substr(1);
                            strs = str.split('&');
                            var stk= new Array();
                            for(var i = 0; i < strs.length; i ++) {
                                stt=theRequest[strs[i].split("=")[0]]=unescape(strs[i].split("=")[1]);
                                stk.push(stt);
                            }

                            //分配保存opendistribute
                            //获取用户id
                            var rows = $.common.isEmpty(iframeWin.$.table._option.uniqueId) ? iframeWin.$.table.selectFirstColumns() : iframeWin.$.table.selectColumns(iframeWin.$.table._option.uniqueId);
                            if (rows.length == 0) {
                                $.modal.alertWarning("请至少选择一条记录");
                                return;
                            }else if (rows.length != 1) {
                                $.modal.alertWarning("只能选择分配给一个用户！");
                                return;
                            }
                            var data = {"dataid":stk[0] ,"rid": rows.join(),"tablename":stk[1] };
                            //提交分配信息
                            var url = $.table._option.distributeUrl;
                            $.operate.saveTab(url, data);
                            //修改当前数据状态
                            var data1 = {"ids":stk[0] };
                            var url1 = $.table._option.removeUrl;
                            $.operate.submit(url1, "post", "json", data1);
                        }
                    },
                    cancel: function(index) {
                        return true;
			...
			//新的弹窗方式
            recoveryOpen: function(title, url, width, height, callback){
				//如果是移动端，就使用自适应大小弹窗
                if (navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)) {
                    width = 'auto';
                    height = 'auto';
                ...
                var shunxuindex = layer.open({
                    ...
                    yes: function(){
                        if(arry.indexOf(idindexvalue) == 0){
                            ...
                            $.operate.edit(arry[arry.indexOf(idindexvalue)-1]);
                        }
                    },
                    btn2: function(){
                        if(arry.indexOf(idindexvalue) == arry.length-1){
        ...
        operate: {
            ...
            // 批量分配信息
            distribute: function() {//selectColumns
                var rows = $.common.isEmpty($.table._option.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns($.table._option.uniqueId);
                str=rows.join();
                //为数据id添加单引号，适配oracle的in方法
                function addQuotationMarks(str){
                ...
                $.modal.confirm("确认要分配选中的" + rows.length + "条数据吗?", function() {
                    var tablename='basp.ggfc_hb';
                    var url = $.table._option.distributeUrl+'#ids='+addQuotationMarks(str)+'&tablename='+tablename;
                    $.modal.opendistribute("分配" + $.table._option.modalName, url);
                });
            },
            examine: function(id) {//审核
                var rows = $.common.isEmpty($.table._option.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns($.table._option.uniqueId);
                if (rows.length == 0) {
                    $.modal.alertWarning("请至少选择一条记录");
                    return;
                }else if($.common.isEmpty(id) && $.table._option.type == table_type.bootstrapTreeTable) {
                    var row = $('#' + $.table._option.id).bootstrapTreeTable('getSelections')[0];
                    var url = $.table._option.examineupUrl.replace("{id}", row[$.table._option.uniqueId]);
                    $.modal.openexamine("审核" + $.table._option.modalName, url);
                } else {
                    $.modal.openexamine("审核" + $.table._option.modalName, $.operate.examineupUrl(id));
                }
            },
            // 清空信息
            clean: function() {
            ...
            recovery: function(id) {
                if($.common.isEmpty(id) && $.table._option.type == table_type.bootstrapTreeTable) {
                    ...
                    var url = $.table._option.updateUrl.replace("{id}", row[$.table._option.uniqueId]);
                    $.modal.recoveryOpen("修改" + $.table._option.modalName, url);
                } else {
                    $.modal.recoveryOpen("修改" + $.table._option.modalName, $.operate.editUrl(id));
                }
            },
            // 修改信息，以tab页展现
            editTab: function(id) {
            ...
            // 审核访问地址
            examineupUrl: function(id) {
                var url = "/404.html";
                if ($.common.isNotEmpty(id)) {
                    url = $.table._option.examineupUrl.replace("{id}", id);
                } else {
                    var id = $.common.isEmpty($.table._option.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns($.table._option.uniqueId);
                    if (id.length == 0) {
                        $.modal.alertWarning("请至少选择一条记录");
